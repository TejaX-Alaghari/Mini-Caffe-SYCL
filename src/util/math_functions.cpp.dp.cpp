...
void caffe_copy(const int N, const real_t *X, real_t *Y) try {
  std::cout << "Size of mem in caffe_copy: " << (sizeof(real_t) * N) << "B" << std::endl;
  ...
  if (X != Y) {
    if (Caffe::mode() == Caffe::GPU) {
      ...
      */
      CUDA_CHECK(
          (dpct::get_default_queue().memcpy(Y, X, sizeof(real_t) * N).wait(),
           0));
      /*
      CUDA_CHECK(
          (dpct::get_default_queue().memcpy(Y, X, sizeof(real_t) * N),
           0));
      */
#else
      NO_GPU;
      ...
      std::cout << "Using CPU memcpy" << std::endl;
