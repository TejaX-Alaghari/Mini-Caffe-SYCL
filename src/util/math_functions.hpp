...
inline void caffe_gpu_memset(const size_t N, const int alpha, void *X) try {
  /*
  ...
  */
  ...
  may need to rewrite this code.
  */
  // CUDA_CHECK((dpct::get_default_queue().memset(X, alpha, N).wait(),
  //            0)); // NOLINT(caffe/alt_fn)
  CUDA_CHECK((dpct::get_default_queue().memset(X, alpha, N),
              0)); // NOLINT(caffe/alt_fn)
}
